...
public class AdminDAOImpl implements AdminDAO {
	...
	public List<Admin> getAdminByName(String adminName) {
		...
		List<Admin> resultList = session.createQuery("select a from Admin a where a.adminName=?1")
				.setParameter(1, adminName).getResultList();
	...
	public List<Admin> getAdminByRole(String adminRole) {
		Session session = sessionFactory.getCurrentSession();
		@SuppressWarnings("unchecked")
		List<Admin> resultList = session.createQuery("select a from Admin a where a.adminRole=?1")
				.setParameter(1, adminRole).getResultList();
		return (resultList.isEmpty() ? null : resultList);
	}
